...
def edit_inform(request):
    if request.method == 'GET':
        uid = request.session.get('uid')
        return render(request, 'edit-inform.html', {'user': models.User.objects.filter(uid=uid).all()[0]})
...
def my_likes(request):
    if request.method == 'GET':
        ...
        fav_list=fmod.filter(c_uid=uid)
        response={
            'favorite_list':fav_list
        }
        return render(request,'likes.html',response)
        #return render(request, 'likes.html', {'CltNovel': models.CltNovel.objects.filter(c_uid=uid).all()})
...
def handle(request):
    try:
        ...
        uid=request.session.get('uid')
        fav_list=CltNovel.objects.filter(c_uid=uid)
        if fav_list.filter(c_tid=tid):
            fav_list.filter(c_tid=tid).delete()
            return HttpResponse('♡')
        else:
            try:
                topic = models.Topic.objects.get(id=tid)
                ...
                return redirect('/home')
            c_tphoto=topic.t_photo
            c_ttitle=topic.t_title
            c_tintroduce=topic.t_introduce
            fav = {
                'c_tid':tid,
                'c_uid':uid,
                'c_tphoto':c_tphoto,
                'c_ttitle':c_ttitle,
                'c_tintroduce':c_tintroduce
            }
            models.CltNovel.objects.create(**fav)
            return HttpResponse('♥')
